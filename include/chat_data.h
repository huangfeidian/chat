...
namespace spiritsaway::system::chat
{
	...
	public:
		...
		const std::uint32_t m_record_num_in_doc;
		const std::uint64_t m_create_ts;
		chat_data_proxy(const std::string chat_key, chat_data_init_func init_func, chat_data_load_func load_func, chat_data_save_func save_func, std::uint32_t record_num_in_doc = 50);
	private:
		struct chat_fetch_task
		{
			// 这里两个是闭区间
			std::uint32_t chat_seq_begin;
			std::uint32_t chat_seq_end;
	...
	public:
		...
		bool fetch_records(std::uint32_t seg_begin, std::uint32_t seq_end, std::vector<chat_record>& result) const;
		void fetch_records(std::uint32_t seq_begin, std::uint32_t seq_end, std::function<void(const std::vector<chat_record>&)> fetch_cb);
		std::uint32_t add_chat(const std::string& from_player_id, const json::object_t& chat_info);
		void add_chat(const std::string& from_player_id, const json::object_t& chat_info, std::function<void(std::uint32_t)> add_cb);
		bool save();
