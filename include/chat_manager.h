...
namespace spiritsaway::system::chat
{
	...
	private:
		void fetch_history_num_cb(chat_data_proxy& proxy_data, std::function<void(std::uint32_t)> num_cb);
		void add_msg_cb(std::shared_ptr<chat_data_proxy> cur_data, std::uint32_t msg_seq, std::function<void(std::uint32_t)> seq_cb);
		std::shared_ptr<chat_data_proxy> get_or_create_chat_data(const std::string& chat_key);
	public:
		chat_manager(chat_data_init_func init_func, chat_data_load_func load_func, chat_data_save_func save_func, std::uint32_t record_num_in_doc = 50);
		void add_msg(const std::string& chat_key, const std::string& from, const json::object_t& msg, std::function<void(std::uint32_t)> seq_cb);
